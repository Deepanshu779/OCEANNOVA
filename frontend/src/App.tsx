...
const demoVessels = [
  { name: "OCEAN STAR", score: 96.2, distance: 12.4, time: 2.1, match: "High", anomaly: "Yes" },
  { name: "SEA HORIZON", score: 87.7, distance: 28.6, time: 3.4, match: "High", anomaly: "Yes" },
  { name: "MARINE EXPRESS", score: 75.6, distance: 42.1, time: 5.2, match: "Medium", anomaly: "Possible" },
  { name: "COASTAL TRADER", score: 54.3, distance: 67.8, time: 8.6, match: "Low", anomaly: "No" },
];
  ...
  return (
    <div className={`command-app ${dark ? "theme-dark" : "theme-light"}`}>
      <header className="command-topbar">
        ...
        <div className="command-actions"><button className="mode-badge" onClick={() => setDark(!dark)}><i>✓</i><span>{dark ? "Dark Mode" : "Light Mode"}<small>Theme</small></span></button><button className="icon-button" aria-label="Toggle theme" onClick={() => setDark(!dark)}>{dark ? "☀" : "☾"}</button><div className="user-chip"><b>DP</b><span>Deepanshu<small>Team Lead</small></span></div></div>
      </header>
      ...
      <div className="command-body">
        ...
        <main className="investigation-workspace">
          ...
          <section className="bottom-evidence">
            ...
            <article className="evidence-card"><div className="evidence-card-title"><span className="card-icon cyan">●</span><strong>Analysis Status</strong><b className="complete">Complete</b></div><div className="status-list"><span>✓ Image Processing</span><span>✓ Spill Detection (AI)</span><span>✓ Vessel Correlation</span><span>✓ Evidence Analysis</span></div></article>
          </section>
        </main>
      </div>
