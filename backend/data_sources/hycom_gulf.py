...
Source: https://ncss.hycom.org/thredds/catalogs/GOMb0.04/reanalysis.html
"""
...
def _fetch_csv(base_url: str, variables: str, latitude: float, longitude: float, timestamp: datetime, *, include_depth: bool = False) -> list[dict[str, str]]:
    params = {
        "var": variables,
...
def fetch_point_forcing(latitude: float, longitude: float, timestamp: datetime) -> HYCOMForcing:
    ...
    current_rows = _fetch_csv(HYCOM_3Z_2018, "u,v", latitude, longitude, timestamp, include_depth=True)
    ...
    current, wind = current_rows[0], wind_rows[0]
    return HYCOMForcing(
...
def fetch_series(latitude: float, longitude: float, start: datetime, hours: int, step_hours: int = 1) -> list[HYCOMForcing]:
    """Fetch a stepped forcing series for drift hindcast or forecast."""
